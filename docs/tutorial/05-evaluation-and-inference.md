...
The project format does not include crowd regions. To compare with a dataset's published score, convert its labels and predictions using that dataset's rules and run its evaluator. The built-in evaluator is for this repository's non-crowd masks.
...
The CLI can write a JSON report next to the run:
...
The report includes the checkpoint SHA-256, requested and resolved device, split, data fingerprint, aggregate metrics, per-image metrics, and a lowest-PQ list. Inspect semantic confusion, missing centers, duplicate centers, merged/split objects, tiny-region filtering, and offset direction separately.
