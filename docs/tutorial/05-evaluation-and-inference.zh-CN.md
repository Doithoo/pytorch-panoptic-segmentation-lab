...
评估首先检查 prepared manifest，并把 `dataset.yaml` 的 identity 与 checkpoint identity 对比。之后才会安全重建保存的架构、加载 tensor state、应用保存的后处理配置并评估 prepared split。`max_test_samples` 与 valid 上限相互独立。
...
报告包含 checkpoint SHA-256、请求与解析后的设备、split、prepared-data identity、总体指标、每图指标和最低 PQ 失败样本 shortlist。
